<x-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="flex items-center mt-5">
            <x-button wire:click="confirmLogout" wire:loading.attr="disabled" class="bg-blue-600 hover:bg-blue-700 text-white">
                {{ __('Log Out Other Browser Sessions') }}
            </x-button>

            <x-action-message class="ms-3 text-green-500" on="loggedOut">
                {{ __('Done.') }}
            </x-action-message>
        </div>
        ...
        <x-dialog-modal wire:model.live="confirmingLogout">
            ...
            <x-slot name="footer">
                <x-secondary-button wire:click="$toggle('confirmingLogout')" wire:loading.attr="disabled" class="bg-gray-500 text-white">
                    {{ __('Cancel') }}
                </x-secondary-button>

                <x-button class="ms-3 bg-red-600 hover:bg-red-700 text-white" wire:click="logoutOtherBrowserSessions" wire:loading.attr="disabled">
                    {{ __('Log Out Other Browser Sessions') }}
                </x-button>
            </x-slot>
        </x-dialog-modal>
